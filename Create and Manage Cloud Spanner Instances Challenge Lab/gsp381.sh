...
gcloud spanner instances create banking-ops-instance --config=regional-$REGION --description="sparkwave" --nodes=1
...
curl -LO https://raw.githubusercontent.com/imharshtiwari/2-Minutes-GCP-Lab-Solutions/main/Create%20and%20Manage%20Cloud%20Spanner%20Instances%20Challenge%20Lab/Customer_List_500.csv
...
gsutil cp emptyfile gs://$DEVSHELL_PROJECT_ID/tmp/sparkwave
...
gcloud dataflow jobs run sparkwave --gcs-location gs://dataflow-templates-"$REGION"/latest/GCS_Text_to_Cloud_Spanner --region="$REGION" --staging-location gs://$DEVSHELL_PROJECT_ID/tmp/ --parameters instanceId=banking-ops-instance,databaseId=banking-ops-db,importManifest=gs://$DEVSHELL_PROJECT_ID/manifest.json
